...
public class QueueListFragment extends SongListFragment {
    ...
    private static final String TAG = LogHelper.INSTANCE.makeLogTag(QueueListFragment.class);
    ...
    public void onMediaBrowserChildrenLoaded(@NonNull String parentId,
        ...
        if (children != null && !children.isEmpty()) {
            String baseMediaId = children.get(0).getDescription().getExtras().getString(MediaItemHelper.META_DATA_KEY_BASE_MEDIA_ID);
            ProviderType providerType = MediaIDHelper.INSTANCE.getProviderType(baseMediaId);
            if (providerType != null && baseMediaId != null) {
                mAdapter.showLayoutInfo(
                        new QueueTitleItem(providerType, MediaIDHelper.INSTANCE.extractBrowseCategoryValueFromMediaID(baseMediaId)), true);
            }
        }
